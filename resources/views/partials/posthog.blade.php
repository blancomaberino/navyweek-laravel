{{-- PostHog product analytics — ported from src/components/PostHog.astro. Inline
     CDN array-stub in <head>; the project key is a public client-side key by
     design. Config (key/host/ui_host) is env-overridable via config/site.php. --}}
@php($posthog = config('site.posthog'))
@if (! empty($posthog['key']))
    {{-- PostHog --}}
    <script>
        ...
        posthog.init(@json($posthog['key']), {
            api_host: @json($posthog['host']),
            ui_host: @json($posthog['ui_host']),
            defaults: '2026-05-30',
