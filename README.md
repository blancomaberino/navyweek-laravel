...
| Repository | `Publishing\Repositories\PageRepositoryInterface` → `EloquentPageRepository` | Publishing | Page lookups on `url_path`: `publishedPathExists` (middleware route gate), `findPublishedByPath` (render read, `pageable` eager-loaded), and `liveDiscountBrandPagesForConnections` (the category hub's live-brand grid). |
