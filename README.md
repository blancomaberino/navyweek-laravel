...
| Model | `App\Models\User` | (root) | A login account that doubles as the editorial byline. Beyond auth, carries the PUBLIC author profile the discount-guide `Person` JSON-LD + `/authors/{slug}/` page read (`slug` → `/authors/{slug}/`, `job_title`, `credentials`, `avatar_path`, `knows_about`, `bio`, `linkedin_url`); `authoredPages`/`reviewedPages` are the inverse of `Page.author`/`reviewer`. Replaces the legacy hardcoded author/reviewer persons — the byline is assignable per-page from the admin panel, and the profiles themselves are CRUD-managed there via **AuthorResource**. |
