...
| Model | `Publishing\Models\Page` | Publishing | Central routing/SEO row keyed on the canonical `url_path`; the DB successor to the build-time route manifest. Carries the head-meta/OG/robots SEO layer, build-clock Article dates, a `json_ld` slot for page-specific extra schema, the polymorphic `pageable` owner (Offer/Connection/pillar), and the editorial byline (`author_id`/`reviewer_id` → `users`, nullable) that drives the guide's Article `author` + WebPage `reviewedBy` Person JSON-LD. |
...
| Repository | `Catalog\Repositories\LocalDiscountRepositoryInterface` → `EloquentLocalDiscountRepository` | Catalog | Local-page reads: `find` (state/city/business triple), the `forState`/`forCity` rollups, and `all` (stores+hours eager-loaded — the detail-page generation sweep). |
