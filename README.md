...
| Model | `App\Models\User` | (root) | A login account that doubles as the editorial byline. Beyond auth, carries the PUBLIC author profile the discount-guide `Person` JSON-LD + the `/authors/{slug}/` profile page read (`slug` → `/authors/{slug}/`, `job_title`, `credentials`, `avatar_path`, `knows_about`, long-form `bio`, `linkedin_url`, plus the full public profile the `/authors/{slug}/` page renders: `service_title`/`current_title`, `location_*`, the `military_timeline`/`civilian_timeline` entry lists, `profile_expertise`, the `expertise_lead`/`works_lead` lead-ins, `featured_works`, and `profile_reviewed_at`); `authoredPages`/`reviewedPages` are the inverse of `Page.author`/`reviewer`, and `authorProfilePage` is the user's own `/authors/{slug}/` page. Replaces the legacy hardcoded author/reviewer persons — the byline is assignable per-page from the admin panel, and the profiles themselves are CRUD-managed there via **AuthorResource**. |
...
Supporting types: value objects `Shared\ValueObjects\UrlPath` and `Publishing\Support\FaqItem` (a computed, non-persisted FAQ pair for pages whose FAQs derive from live data, e.g. the free-meals roundup); the page-path knob `Publishing\Support\PagePaths` (reads `config('publishing.paths.*')` — the single place a family prefix like `/navy-bases/` is defined; both the generators and the SEO schemas build family/ancestor/child links through it, and every schema derives its own `@id`/canonical from `$page->url_path`); the map-geometry assets `Publishing\Support\UsMapGeometry` (Albers-USA landmass/state-border paths + tour-stop pin positions, ported from `src/data/usMapGeometry.ts`, used by `/map/` and the home map teaser) and `Pillars\Support\BaseMapSvg` (the separate hand-drawn bases projection — server-rendered inline map SVGs ported from `USBasesMap.tsx` + `WorldMapSVG.tsx`, used by the base detail, country hub and overseas hub views); the category-ordering rule `Catalog\Support\DiscountCategoryOrdering` (port of the legacy `orderCategoryDiscounts` — applies a hub's `pinned`/`order`/`excluded`, which hold PAGE slugs, so it runs in the layer holding the page rather than in the Catalog repository, which only sees connection slugs); services `Publishing\Services\LegacyPathResolver`, `Catalog\Services\AffiliateLinkTagger` (port of `withPlacement` — the outbound sub-ID tagging choke point), `Publishing\Sitemap\SitemapGenerator` (hand-port of `generate-sitemap.mjs` — buckets `allPublishedIndexable` pages by url-path into the 9-file split + index, pure/returns `SitemapResult` for `sitemap:generate` to write), `Publishing\Feed\FeedGenerator` (hand-port of `generate-llm-feed.mjs` — assembles `llms.txt` + the `navy-week-2026.json` feed from every pillar/catalog aggregate, pure/returns `FeedResult` for `feed:generate` to write); page-schema serializers `Publishing\Seo\{HomePageSchema, DiscountGuideSchema, DiscountCategorySchema, DiscountIndexSchema, LocalDiscountSchema, VeteransDayFreeMealsSchema}` + pillar `*PageSchema` builders, all on the shared `BuildsSeoSchema` trait (which also holds the author/reviewer `Person` builders reused by the guide + local-business detail graphs, and the `webSite`/`usNavyOrganization`/`navcoOrganization` nodes shared by the home landing + Navy Week city graphs); the DB-driven-content-page serializer `Publishing\Seo\ContentPageSchema` (Breadcrumb-only, Article+Person+FAQPage (veterans-day), or Article+Person×2+WebPage (va-disability/veterans-home-care, follow-up) by flags) + view `pages.content` (renders `body_blocks`, edited in the PageForm **Content body** Builder — `Filament\Resources\Pages\Schemas\ContentBlocks`, one block per renderable type); the CMS translation layer `Publishing\Content\BodyBlocks` (stored blocks ↔ Builder state, lossless + shape-preserving, applied on the page's fill/save hooks by `TranslatesBodyBlocks`) and `Publishing\Content\InlineSpans` (the inline-run vocabulary in both directions — `render()` is the PAGE html the content view + base hubs emit, `toHtml()`/`fromHtml()` the editor's); page generation `Publishing\Pages\GenerateHomePageAction` (+ `pages:generate-home` — seeds the site root `/`, a data-driven hub whose body is the live schedule from the pillar; seeds the home FAQs, never a body), `Catalog\Pages\GenerateLocalDiscountPagesAction` (+ `pages:generate-local-discounts`), `Publishing\Pages\GenerateContentPagesAction` (+ `pages:generate-content` — seeds privacy/terms/contact) and `Publishing\Pages\GenerateVeteransDayPageAction` (+ `pages:generate-veterans-day` — migrates the prose + 9 FAQs), both never clobbering an editor's body; the **author-profile page family** — one `/authors/{slug}/` page per byline user (the `authors` path family in `config('publishing.paths')`) — `Publishing\Seo\AuthorPageSchema` (Person + Breadcrumb + ProfilePage + an authored-articles ItemList, derived from `$page->url_path`) + view `pages.author`, generated by `Publishing\Pages\GenerateAuthorPagesAction` (+ `pages:generate-authors`, keyed on the immutable `author:{user-id}` — slug is mutable location, not identity — pageable = the `User`) over `AuthorRepositoryInterface::publicProfiles`, dispatched via `PageType::Author` → `PageController::renderAuthor`; the data-driven `/veterans-day/free-meals/` roundup — `Publishing\Seo\VeteransDayFreeMealsSchema` + `Catalog\Support\VeteransDayFreeMealsPresenter` (live stats/ItemList/FAQ computed from the `verified()` meals) + view `pages.veterans-day-free-meals`, seeded as a `Static` slug page by `Publishing\Pages\GenerateVeteransDayFreeMealsPageAction` (+ `pages:generate-veterans-day-free-meals`) and dispatched via `renderStatic` slug `veterans-day-free-meals`; and the pillar `Generate*PagesAction`s; the editable-URL loop `Publishing\Actions\ChangeUrlPathAction` → `Publishing\Events\PageUrlChanged` → `Publishing\Listeners\CreateRedirectListener` (renaming a page auto-writes its 301 + collapses chains, no deploy — wired in `DomainServiceProvider::boot`); enums `Crm\Enums\{ConnectionStatus,Audience}`, `Catalog\Enums\{OfferType,VerificationProvider,RedemptionChannel,Placement,MealEligibility,MealRedemption,MealStatus,LocalVerification}`, `Research\Enums\{ResearchStatus,ResearchedBy}`, `Shared\Enums\{ConfidenceLevel,SourceType}` (confidence is shared by briefs + citations), `Pillars\Enums\{BaseType,CombatantCommand,RegionType,RankCategory,DesignatorCommunity,RatingCommunity,HistoricRatingEra,NavyWeekStatus,NavyWeekSourceLevel,FleetWeekSeason,FleetWeekStatus,AirShowStatus,Admission,TeamId,JetTeamStatus}`, `Publishing\Enums\{PageType,RedirectMatchType}`. Seeders: `AffiliateNetworkSeeder` (the 7 networks), `AudienceSeeder` (audience vocabulary from the enum), `EditorialTeamSeeder` (the two default byline `users` — author + reviewer — matching `config('site.editorial.*')`; back-fills the default byline onto any page missing one).

### Parity commands (site families + content import)

| Command | What |
|---|---|
| `pages:generate-base-hubs` | `/navy-bases/` root, overseas, per-state and per-country hubs |
| `pages:generate-designators` | Officer-designator hub, 3 community hubs, 24 detail pages |
| `pages:generate-discount-categories` | The 5 `/discount/{category}/` hubs |
| `pages:generate-navy-reference` | `/navy-reference/` library landing page |
| `pages:generate-schedule` | `/schedule/` and `/map/` |
| `import:content-bodies` | Long-form page bodies → `pages.body_blocks` |
| `import:content-rich-bodies` | The same bodies with their tables, callouts, jump navs, FAQ answers, contact cards and inline source links → `pages.body_blocks` (plus the policy pages' `eyebrow`/`h1`) |
| `import:content-page-meta` | Content-page hero/trust meta → `pages.{key_facts,disclosure,eyebrow,shows_reference_backlink}` + their FAQs |
| `import:discount-details` | "How it works" paragraphs → `offers.details` |
| `import:author-profiles` | Author-profile timelines, titles, location → `users.*` |
| `backfill:discount-trust` | Trust CMS columns on the 981 discount pages |
