...
The two consolidated reference lists — **`rank`** (`/navy-ranks/`) and **`rating`**
(`/navy-ratings/`, a new `PageType::Rating`) — each list every entry on one page with
per-entry anchors (`/navy-ranks/#ensign`), so the page owns **no `pageable`** (the
`match` arm renders directly, no `instanceof` guard). `RankListSchema` (shared, path/
crumb/ItemLists parameterized) emits Breadcrumb + Article + two `ItemList`s per page —
officer (commissioned+warrant) & enlisted for ranks; active & historic for ratings —
each ListItem linking the in-page anchor, ported from `buildRanksItemListSchema`. The
`pages.rank-list`/`pages.rating-list` views render paygrade-ordered sections
(`RankRepository::forCategoryByPaygrade` — numeric, not lexical) and community-grouped
ratings. Both pages are generated by **`GenerateRankPagesAction`** (`pages:generate-ranks`)
via `PageRepository::upsertPillarPage` (now accepts a **null** pageable for list/hub pages).

The **`air_show`** type covers both the guide (`/air-show/{slug}/`, pageable →
`AirShow`) and the hub (`/air-show/`, pageable → `AirShowHubMeta`) — the `match` arm
splits on the pageable class. `AirShowPageSchema::buildDetail` emits the guide graph
(Breadcrumb + Article + WebPage + author/reviewer Person + FAQPage) plus an **Event**
node gated on `AirShow::emitsEventSchema()` (published, confirmed date, no canonical
override); `buildHub` emits Breadcrumb + Article + ItemList + FAQPage. The
author/reviewer Person nodes are driven by the page byline (the seeded
t-alford/erik-rivera users, whose fields equal the legacy TRUST_AUTHOR/DEFAULT_REVIEWER
constants); a canonical-override show gets a page that canonicalizes to its primary show.
...
- **PageResource** (`Publishing` nav group) — the published-URL registry. Table:
  ...
  **Content body** is a Filament `Builder` (`ContentBlocks`) over `pages.body_blocks`
  ...
  the CMS-backed content pages carry one; a data-driven page (discount, base, event)
  leaves it empty because its body comes from its aggregate.
  ...
  **`InlineSpans`** maps each run-list ↔ HTML. The translation is lossless and
