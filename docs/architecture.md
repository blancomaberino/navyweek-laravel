...
Last updated: Phase 3 rendering — reference pillars. Base pages (`/navy-bases/{slug}/`,
`BasePageSchema` + `pages.base` + `pages:generate-bases`) and the consolidated
ranks/ratings list pages (`/navy-ranks/`, `/navy-ratings/` — `RankListSchema` +
`pages.rank-list`/`pages.rating-list` + `pages:generate-ranks`, new `PageType::Rating`).
Data model unchanged since Phase 2 slice 10b; these slices add only the
rendering/page-generation path (`PageRepository::upsertPillarPage`, now null-pageable-aware).
